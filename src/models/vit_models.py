...
class ViT(nn.Module):
    ...
    def __init__(self, cfg, load_pretrain=True, vis=False):
        ...
        if cfg.MODEL.TRANSFER_TYPE != "end2end" and "prompt" not in cfg.MODEL.TRANSFER_TYPE:
            # linear, cls, tiny-tl, parital, adapter, lora
            self.froze_enc = True
        else:
        ...
        self.build_backbone(prompt_cfg, cfg, adapter_cfg, lora_cfg, load_pretrain, vis=vis)
        self.cfg = cfg
        self.setup_side()
        self.setup_head(cfg)
    ...
    def build_backbone(self, prompt_cfg, cfg, adapter_cfg, lora_cfg, load_pretrain, vis):
...
class SSLViT(ViT):
    ...
    def build_backbone(self, prompt_cfg, cfg, adapter_cfg, lora_cfg, load_pretrain, vis):
        ...
        self.enc, self.feat_dim = build_fn(
            cfg.DATA.FEATURE, cfg.DATA.CROPSIZE,
            prompt_cfg, cfg.MODEL.MODEL_ROOT, adapter_cfg=adapter_cfg, lora_cfg=lora_cfg
        )
